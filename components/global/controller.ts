...
function getItemDataForMods(modIds:string[]){
  interface Mods{
    slotName:string,
    iconLink:string,
    shortName:string,
...
selectDataByColumn(['id','postId'],'posts',null);

//원하는 컬럼의 데이터를 SELECT하고 값을 반환하는 함수
//매개변수로 컬럼 배열과 데이터베이스 이름을 받는다.
function selectDataByColumn(column:string[],table:string,where:string|null){
  let sql='';
  sql=combineStringIntoQuery(column,table,where);
  sendQuery(sql).then((data)=>{
    console.log(data);
    return data;
  });
...
function combineStringIntoQuery(column:string[],table:string,where:string|null,value:string|null):string{
...
async function sendQuery(sql:string){
  ...
  try {
    return await new Promise((resolve, reject) => {
      connection.query(sql, null, (error:Error, results:object[]) => {
        if (error) {
          reject(error);
        } else {
          resolve(results);
